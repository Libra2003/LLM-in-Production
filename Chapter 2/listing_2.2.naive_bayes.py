...
mr_corpus = PlaintextCorpusReader("./", r".*\.txt")

sents = mr_corpus.sents(fileids="C:/Users/hp envy/OneDrive/Desktop/LLM IN Production/data/hamlet.txt")
...
def count_utts(result, utts, ys):
    ...
    Input:
        ...
        utts: a list of utts
        ys: a list of sntiment of each utt (either 0 or 1)
...
print(lookup(freqs, "be", True))
# for k, v in freqs.items():
#     if "b" in k[0]:
#         print(f"{k}:{v}")

def train_naive_bayes(freqs, tran_x, train_y):
    """
    Input:
    ...
    # calculate V, the number of unique words in th vocabulary
    vocab = set([pair[0] for pair in freqs.keys()])
    V = len(vocab)

    #calculate N_pros and N_neg
    N_pos = N_neg = 0
    for pair in freqs.keys():
        ...
        if pair[1] > 0:
            ...
            #incrment the number of ngative words (word, label)
            N_neg += lookup(freqs, pair[0], False)

    # Calculate D, the number of documents
    D = len(train_y)

    # Caclculate the number of positivee documents
    D_pos = sum(train_y)

    # Caclculate the number of negative documents
    D_neg = D - D_pos
    ...
    for word in vocab:
        ...
        # calcualate the log likelihood of the word
...
def naive_bayes_predict(utt, logprior, loglikelihood):
    ...
    # process th utt to get a list of words
    word_1 = process_utt(utt)
    ...
    for word in word_1:
        # check f word exists in the logliklihood dictionary'
        if word in loglikelihood:
            # add the log likelihood of that word to that probability
...
def test_naive_bayes(test_x, test_y, logprior, loglikelihood):
    ...
    for utt in test_x:
        ...
        if naive_bayes_predict(utt, logprior, loglikelihood) > 0:
            # the predictd class is 1
            y_hat_i = 1
        else:
            # otherwise the predicted class is 0
            y_hat_i = 0

        # append th epredicted class to list y_hats
        y_hats.append(y_hat_i)


    # error = avg of the abs vals of the diffs betwen y_hats and test_y
    error = sum(
