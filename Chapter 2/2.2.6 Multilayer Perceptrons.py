    ...
    def __init__(
        ...
        for _ in range(num_hidden_layers):
            self.modules_list.append(
                nn.Linear(interim_input_size, output_size)
            )
            self.modules_list.append(hidden_activation())
            interim_input_size = interim_output_size
        self.fc_final = nn.Linear(interim_input_size, interim_output_size)

        self.last_forwar_cache = []
