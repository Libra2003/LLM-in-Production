...
# To exactly this:
embs_train = load_dataset("cardiffnlp/tweet_eval", "emoji", split="train[:1000]")
embs_test = load_dataset("cardiffnlp/tweet_eval", "emoji", split="test[:100]")
...
for score, label, text in zip(scores, samples["label"], samples["text"]):
    print("=" * 50)
    print(f"TEXT:\n{text[:200]} [...]")
    print(f"SCORE: {score:.2f}")  # <--- Remove the 's' right here
    print(f"LABEL: {label}")
